...
window.sendComment = () => { if(!curItem || !curItem.id) { Swal.fire("Error", "ID Postingan tidak valid.", "error"); return; } const input = document.getElementById('comment-input'); const txt = input.value.trim(); if(!txt) return; const finalUsername = user.username || user.name || "User"; const cId = Date.now().toString(); const path = `interactions/comments/${curItem.id}/${cId}`; const data = { id: cId, text: txt, username: finalUsername, pic: user.profile_pic || user.pic || "https://ui-avatars.com/api/?name=" + finalUsername, timestamp: serverTimestamp() }; set(ref(rtdb, path), data).then(() => { input.value = ''; showGameToast("Terkirim", "success"); }).catch((e) => { Swal.fire("Gagal Kirim", e.message, "error"); }); }
